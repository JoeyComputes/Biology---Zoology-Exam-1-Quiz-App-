...
# Functions to display the current questions and choices
def show_question():
    global current_question
    question = quiz_data[current_question]
    qs_label.config(text=question["question"])

    # Displays the choices on the buttons
...
def check_answer(choice):
    ...
    if selected_choice == question["answers"]:
        score += 1
    ...
    # Disable all the choice buttons and enable the next Button
...
def next_question():
    ...
    if current_question < len(quiz_data):
        ...
        messagebox.showinfo("Biology Quiz Completed", "Your score is {}/{}".format(score, len(quiz_data)))
        root.destroy()
